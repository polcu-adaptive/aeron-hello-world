...
public class CliAgent implements Agent
{
    ...
    private final TakeSnapshotCommandEncoder snapshotCommand = new TakeSnapshotCommandEncoder();
    ...
    private void sendMessages()
    {
        ...
        switch (message)
        {
            case "snapshot" ->
            {
                snapshotCommand.wrapAndApplyHeader(unsafeBuffer, 0, headerEncoder);
                length = headerEncoder.encodedLength() + snapshotCommand.encodedLength();
            }
            default ->
            {
                messageEncoder.wrapAndApplyHeader(unsafeBuffer, 0, headerEncoder);
                messageEncoder.message(message);
                length = headerEncoder.encodedLength() + messageEncoder.encodedLength();
            }
        }
